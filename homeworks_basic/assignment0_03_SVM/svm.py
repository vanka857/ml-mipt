...
def hinge_loss(scores, labels):
    ...
    return torch.max(torch.zeros_like(scores), 1 - scores*labels).mean()### YOUR CODE HERE
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = k_batch @ self.betas + self.bias ### YOUR CODE HERE 
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return torch.squeeze(torch.tanh(K @ self.betas + self.bias)) ### YOUR CODE HERE
